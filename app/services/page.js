...
import { MessageCircle, Phone, Shirt, WashingMachine, Wind, Bed, Layers, Crown, Briefcase, Wand2, Scissors, Home as HomeIcon, ArrowRight } from 'lucide-react'
...
async function loadServices() {
  ...
  return db.collection(COLLECTIONS.services).find({ active: true }).sort({ display_order: 1 }).toArray()
}
      ...
      <section className="section" style={{ background: 'linear-gradient(180deg, #F5F9FC 0%, #FFF 100%)' }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-3">
            <a href={waLink()} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-md px-5 py-3 text-sm font-semibold text-white shadow-sm" style={{ background: BRAND.green }}><MessageCircle className="h-4 w-4" /> Book Pickup on WhatsApp</a>
            <Link href="/price-list" className="inline-flex items-center justify-center gap-2 rounded-md border-2 px-5 py-3 text-sm font-semibold" style={{ borderColor: BRAND.blue, color: BRAND.blue }}>View Price List <ArrowRight className="h-4 w-4" /></Link>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {services.map(s => {
              ...
              return (
                <div key={s._id} className="card-hover group rounded-xl border border-slate-200 bg-white p-6 flex flex-col">
                  <div className="h-11 w-11 rounded-lg flex items-center justify-center mb-4" style={{ background: '#EEF4FB' }}><Icon className="h-6 w-6" style={{ color: BRAND.blue }} /></div>
                  ...
                  <div className="mt-5 flex flex-wrap items-center gap-3">
                    <a href={waEnquire(s.name)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-[13.5px] font-semibold" style={{ color: BRAND.greenDark }}><MessageCircle className="h-4 w-4" /> Enquire on WhatsApp</a>
                    <Link href="/price-list" className="inline-flex items-center gap-1 text-[13px] font-medium" style={{ color: BRAND.blue }}>See prices <ArrowRight className="h-3.5 w-3.5" /></Link>
                  </div>
                </div>
              )
            })}
          </div>
          {services.length === 0 && <div className="text-slate-500 text-center py-10">No active services yet.</div>}
        </div>
      </section>
      ...
      <section className="section" style={{ background: BRAND.bg }}>
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="mt-6 flex flex-col sm:flex-row justify-center gap-3">
            <a href={waLink()} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-md px-5 py-3 text-sm font-semibold text-white" style={{ background: BRAND.green }}><MessageCircle className="h-4 w-4" /> Book Pickup on WhatsApp</a>
