...
/**
 * Reusable pickup lead form. Used by:
 *   - BookPickupModal (header popup)
 *   - /book-pickup standalone landing page
 *
 * Submits to the existing POST /api/leads endpoint.
 * `source` prop is saved with each lead so the admin can identify origin.
 */
export default function PickupForm({
  source = 'header_form',
  onSuccess,
  showSuccessCard = true,
  submitLabel = 'Submit Pickup Request',
}) {
    ...
    return (
      <div className="p-6 text-center">
        ...
        <div className="mt-5 flex flex-col sm:flex-row gap-2">
          <button onClick={reset} className="flex-1 rounded-md border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-800">Submit another</button>
          <a
            ...
            className="flex-1 inline-flex items-center justify-center gap-2 rounded-md px-4 py-2.5 text-sm font-semibold text-white"
  ...
  return (
    <form onSubmit={submit} className="p-5 space-y-3.5" noValidate>
      ...
      <div>
        ...
        <input
          id="bp-phone" type="tel" inputMode="tel" autoComplete="tel" required
          value={form.phone} onChange={update('phone')} placeholder="+91 …"
      ...
      <div>
        ...
        <input
          id="bp-name" type="text" autoComplete="name" value={form.name} onChange={update('name')}
      ...
      <div>
        ...
        <textarea
          id="bp-addr" rows={2} value={form.address} onChange={update('address')}
          className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0759AD]"
        />
      </div>
      <div className="grid grid-cols-2 gap-2.5">
        <div>
          ...
          <input
            id="bp-date" type="date" value={form.date} onChange={update('date')}
            className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0759AD]"
          />
        </div>
        <div>
          ...
          <input
            id="bp-time" type="time" value={form.time} onChange={update('time')}
            className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0759AD]"
          />
        </div>
      </div>
      <button
        ...
        style={{ background: '#42A62B' }}
      >
        {submitting ? <><Loader2 className="h-4 w-4 animate-spin" /> Submitting…</> : submitLabel}
      </button>
